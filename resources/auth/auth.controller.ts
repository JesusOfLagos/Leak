...
export class AuthController {
    ...
    public async Login(req: Request, res: Response) {
        try {
            ...
            const isMatch = await hasher.CompareHashData(password, user.password as string);
            if (!isMatch) {
                return res.status(400).json({
            ...
            const token = await tokenService.generateAccessToken({ userId: userId, email: user.email as string });
            const userData = {
                email: user.email,
    ...
    public async VerifyEmail (req: Request, res: Response) {
        try {
            ...
            const expires = new Date(user.otpExpiresIn as any)
            const now = new Date()
            if (expires < now) {
                return res.status(400).json({
